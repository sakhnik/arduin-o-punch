...
- Режим економії після періоду неактивності
- Глибокий сон після періоду неактивності, пробудження від струсу
- ArduinoOTA
- Картка налагодження для отримання діагностичної інформації
- UART для програмування, конфігурування і налагодження замість USB CDC
- Зчитування, форматування картки через оболонку
- Циклічне форматування картки, зчитування відміток попередніх стартів
- Окрема станція очищення попередніх відміток перед стартом
- Конфігурування станції в додатку Android через BLE
- Поновлення через BLE
- Автодіагностика при запуску

### Змінено

- Ліцензія GPL v2
- FreeRTOS замість кооперативного циклу
- Зменшено тактову частоту до 10 МГц
- Зберігання налаштувань як Preferences в NVS
- Вимкнення BLE і WiFi після 2 хв простою
- Стартова станція не видаляє попередні відмітки
- Журнал відміток зберігатиме час кожної відмітки
- Функціональність WiFi може бути увімкнено під час компіляції
- OK (режим відмітки) просто `ті` (E)

### Видалено

- Використання зовнішьої пам’яті EEPROM
- Очищення відміток у додатку Android
...
- Android: діалог з інформацією про версію і ревізію git
...
- Arduino: періодичне програмне скидання MFRC522
...
[Невипущене]: https://github.com/sakhnik/arduin-o-punch/compare/v2.1.0...HEAD
